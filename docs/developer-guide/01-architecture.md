...
1. `app.whenReady`
2. `initDatabase()` — open `userData/sales.db`, run migrations, seed default permissions
3. Register IPC modules (auth, db, sales, deliveryOrders, stock, reports, financial years, carry-forward, print, …)
4. Load Vite dev URL or production `dist-react` index
