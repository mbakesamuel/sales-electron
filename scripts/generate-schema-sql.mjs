/**
 * Generates 001_init.sql from the Prisma extract translation rules.
 * Run: node scripts/generate-schema-sql.mjs
 */
...
const outPath = path.join(
  __dirname,
  "../src/electron/db/migrations/001_init.sql",
);

const sql = `-- Sales Electron — full schema translated from Prisma/PostgreSQL to SQLite
...
CREATE TABLE IF NOT EXISTS RolePermission (
  ...
  role TEXT NOT NULL CHECK (role IN ('ADMIN','DIRECTOR','MANAGER','OFFICER','SENIOR_SUPERVISOR','SUPERVISOR','CLERK')),
  key TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS RolePermission_key_idx ON RolePermission (key);

CREATE TABLE IF NOT EXISTS GlobalRoleDefinition (
  id TEXT PRIMARY KEY NOT NULL,
  ...
  legacyRole TEXT UNIQUE CHECK (legacyRole IS NULL OR legacyRole IN ('ADMIN','DIRECTOR','MANAGER','OFFICER','SENIOR_SUPERVISOR','SUPERVISOR','CLERK'))
);

CREATE TABLE IF NOT EXISTS GlobalRolePermission (
  id TEXT PRIMARY KEY NOT NULL,
  globalRoleDefinitionId TEXT NOT NULL REFERENCES GlobalRoleDefinition(id) ON DELETE CASCADE,
  key TEXT NOT NULL,
  allowed INTEGER NOT NULL DEFAULT 1 CHECK (allowed IN (0, 1)),
  createdAt TEXT NOT NULL DEFAULT (datetime('now')),
  updatedAt TEXT NOT NULL DEFAULT (datetime('now')),
...
CREATE INDEX IF NOT EXISTS GlobalRolePermission_key_idx ON GlobalRolePermission (key);
...
CREATE TABLE IF NOT EXISTS CommercialServiceRole (
  id TEXT PRIMARY KEY NOT NULL,
  ...
  createdAt TEXT NOT NULL DEFAULT (datetime('now')),
  updatedAt TEXT NOT NULL DEFAULT (datetime('now')),
  requiresFixedPostingSite INTEGER NOT NULL DEFAULT 1 CHECK (requiresFixedPostingSite IN (0, 1)),
  UNIQUE (commercialServiceId, code)
);
CREATE INDEX IF NOT EXISTS CommercialServiceRole_service_active_idx ON CommercialServiceRole (commercialServiceId, isActive);

CREATE TABLE IF NOT EXISTS CommercialServiceRolePermission (
  id TEXT PRIMARY KEY NOT NULL,
  commercialServiceRoleId TEXT NOT NULL REFERENCES CommercialServiceRole(id) ON DELETE CASCADE,
  key TEXT NOT NULL,
  allowed INTEGER NOT NULL DEFAULT 1 CHECK (allowed IN (0, 1)),
  createdAt TEXT NOT NULL DEFAULT (datetime('now')),
  updatedAt TEXT NOT NULL DEFAULT (datetime('now')),
...
CREATE INDEX IF NOT EXISTS CommercialServiceRolePermission_key_idx ON CommercialServiceRolePermission (key);
...
CREATE TABLE IF NOT EXISTS TaxRateSchedule (
  id TEXT PRIMARY KEY NOT NULL,
  taxTypeId TEXT NOT NULL REFERENCES TaxType(id) ON DELETE CASCADE,
  rate TEXT NOT NULL,
  effectiveFrom TEXT NOT NULL,
  createdAt TEXT NOT NULL DEFAULT (datetime('now')),
  updatedAt TEXT NOT NULL DEFAULT (datetime('now')),
  variant TEXT NOT NULL DEFAULT 'DEFAULT' CHECK (variant IN ('DEFAULT','SIMPLIFIED','REAL','NO_TAXPAYER_ID')),
  UNIQUE (taxTypeId, effectiveFrom, variant)
);
CREATE INDEX IF NOT EXISTS TaxRateSchedule_type_from_idx ON TaxRateSchedule (taxTypeId, effectiveFrom);

CREATE TABLE IF NOT EXISTS TaxRegime (
  id TEXT PRIMARY KEY NOT NULL,
  name TEXT NOT NULL,
  vatApplies INTEGER NOT NULL DEFAULT 1 CHECK (vatApplies IN (0, 1)),
  createdAt TEXT NOT NULL DEFAULT (datetime('now')),
  updatedAt TEXT NOT NULL DEFAULT (datetime('now')),
...
CREATE TABLE IF NOT EXISTS TaxRegimeTax (
  taxRegimeId TEXT NOT NULL REFERENCES TaxRegime(id) ON DELETE CASCADE,
  taxTypeId TEXT NOT NULL REFERENCES TaxType(id) ON DELETE CASCADE,
  PRIMARY KEY (taxRegimeId, taxTypeId)
);
CREATE INDEX IF NOT EXISTS TaxRegimeTax_taxType_idx ON TaxRegimeTax (taxTypeId);
...
CREATE TABLE IF NOT EXISTS User (
  id TEXT PRIMARY KEY NOT NULL,
  name TEXT NOT NULL,
  role TEXT NOT NULL DEFAULT 'CLERK' CHECK (role IN ('ADMIN','DIRECTOR','MANAGER','OFFICER','SENIOR_SUPERVISOR','SUPERVISOR','CLERK')),
  isActive INTEGER NOT NULL DEFAULT 1 CHECK (isActive IN (0, 1)),
  ...
  passwordHash TEXT,
  commercialServiceId TEXT REFERENCES CommercialService(id),
  commercialServiceRoleId TEXT REFERENCES CommercialServiceRole(id),
  globalRoleDefinitionId TEXT REFERENCES GlobalRoleDefinition(id),
  createdAt TEXT NOT NULL DEFAULT (datetime('now')),
  updatedAt TEXT NOT NULL DEFAULT (datetime('now'))
);
CREATE INDEX IF NOT EXISTS User_commercialService_idx ON User (commercialServiceId);
CREATE INDEX IF NOT EXISTS User_commercialServiceRole_idx ON User (commercialServiceRoleId);
...
-- Deferred FK for SalesPoint.workingMonthSetById
-- SQLite allows adding FK via table recreation; for fresh schema we recreate SalesPoint link:
-- (already nullable FK without inline constraint — add index only)
CREATE INDEX IF NOT EXISTS SalesPoint_workingMonthSetBy_idx ON SalesPoint (workingMonthSetById);
...
CREATE INDEX IF NOT EXISTS Employee_estate_idx ON Employee (estate);
...
CREATE TABLE IF NOT EXISTS SaleAppliedTax (
  id TEXT PRIMARY KEY NOT NULL,
  saleId TEXT NOT NULL REFERENCES Sale(id) ON DELETE CASCADE,
  taxTypeId TEXT REFERENCES TaxType(id),
