...
import {
  loadAutoGenerateDeliveryOrderNo,
  loadEnforceDeliveryOrderBookletValidation,
...
export function getDeliveryOrdersFormOptions(): DeliveryOrdersFormOptions {
  ...
  return {
    ...
    autoGenerateDeliveryOrderNo: loadAutoGenerateDeliveryOrderNo(),
...
export function saveDeliveryOrder(input: SaveDeliveryOrderInput): SaveDeliveryOrderResult {
  ...
  let deliveryOrderNo: string;
  if (loadAutoGenerateDeliveryOrderNo()) {
    deliveryOrderNo = allocateDeliveryOrderNo(db);
    const duplicate = db
      .prepare(`SELECT 1 AS found FROM DeliveryOrder WHERE deliveryOrderNo = ?`)
      .get(deliveryOrderNo) as { found: number } | undefined;
    if (duplicate) {
      return { ok: false, error: "Could not allocate a unique delivery order number." };
    }
  } else {
    ...
    deliveryOrderNo = serialResult.serial;

    if (loadEnforceDeliveryOrderBookletValidation()) {
