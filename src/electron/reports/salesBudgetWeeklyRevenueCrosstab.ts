import type { SalesBudgetWeeklyRevenueCrosstabReport } from "../../shared/reports.types.js";
import {
...
export function getSalesBudgetWeeklyRevenueCrosstabReport(
  userId?: string | null,
  ...
  >();
  const amountByCell: Array<{ key: string; amountFcfa: number }> = [];
  ...
  for (const cat of context.categoriesInReport) {
    for (const month of CAL_MONTHS) {
      ...
      for (const week of phaseMonth.weeks) {
        ...
        amountMap.set(key, week.amountFcfa);
        amountByCell.push({ key, amountFcfa: week.amountFcfa });
      }
    }
  }
  ...
  const rowTotals = sortedWeeks.map((week) => {
    ...
    for (const col of cols) {
      total +=
        amountMap.get(salesBudgetCrosstabCellKey(week.label, col.productCatId, col.month)) ??
        0;
    }
    return total;
  });
  ...
  return {
    ...
    comments: loadReportComments("sales-budget-weekly-revenue-crosstab"),
  };
}
